...
def add_record():
    ...
    try:
        data_prov.cursor.executemany(
            'INSERT INTO personal VALUES(?,?,?,?,?,?,?)', new_baza)
        data_prov.bd.commit()
    except:
...
def salary_record(id, salary):
    data_prov.cursor.execute(
        f'UPDATE personal SET salary={salary} WHERE id={id};')
    data_prov.bd.commit()
...
def bonus_record(id, bonus):
    data_prov.cursor.execute(
        f'UPDATE personal SET bonus={bonus} WHERE id={id};')
    data_prov.bd.commit()
